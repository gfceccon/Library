...
/*
	Controller
	Controls user, book and loan streaming
	Controls ban dates and error handling
 */
public class LibraryController
{
	...
	{
		if (instance == null) // This is a singleton class since it can only be instantiated once
	...
	//Lend a book, following the user/book rules
		...
		if (user.isPresent())
		{
			if (book.isPresent())
			{
				...
				if (loans    // If the user has already borrowed the maximum ammount of books allowed
						.stream().filter(l -> l.userLogin.equals(userLogin)).filter(l -> l.returnDate == null).count() == user.get().maxBookCount)
	...
	//Return the specify book, setting ban date if necessary
	...
	/*
		SEARCH METHODS BLOCK
	 */
	public List<User> searchUserByName(String name)
	{
		List<User> result = users.stream().filter(u -> u.name.equals(name)).collect(Collectors.toList());
	...
	{
		return users.stream().filter(u -> u.login.equals(login)).findFirst();
	}

	public List<Book> searchBook(String title)
	{
		List<Book> result = books.stream().filter(b -> b.title.equals(title)).collect(Collectors.toList());
